...
- SSH benchmark fix (2026-08-12): `lib/forge/ssh/manager.rb` now passes `CRUKS_SSH_CONFIG` or `~/.ssh/config` with `-F`, avoiding the rejected global SSH config. Focused specs pass 16 examples. Six-model S1 rerun produced raw podman8/podman9 `uname -a` output for Darwin, Veriloop, Qwen3-30B, and Qwen3-latest; DeepSeek planned without executing and Qwen2.5 emitted a tool-call fragment without execution. Benchmark used temporary HOME/audit/session paths; repository remains free of benchmark artifacts.
- S2 runtime-path fix: `CRUKS_DATA_DIR` keeps sessions writable and `CRUKS_AUDIT_PATH` redirects audit logs. This is isolated from the S1 launcher/provider path.
